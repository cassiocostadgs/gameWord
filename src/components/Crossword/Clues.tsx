...
export const Clues: React.FC<CluesProps> = ({ onClueClick, activeDirection, activeCell }) => {
  ...
  const isActive = (clue: Clue) =>
    ...
    activeCell.row < clue.row + (clue.direction === 'col' ? clue.answer.length : 1) &&
    activeCell.col >= clue.col &&
    activeCell.col < clue.col + (clue.direction === 'row' ? clue.answer.length : 1);

  const renderList = (list: Clue[]) =>
    list.map(clue => (
      <li
  ...
  return (
    <aside className="crossword-clues">
      <div className="clues-group">
        <h3>→ Horizontal</h3>
        <ul>{renderList(rowClues)}</ul>
      </div>
      <div className="clues-group">
        <h3>↓ Vertical</h3>
        <ul>{renderList(colClues)}</ul>
      </div>
    </aside>
  );
};
